...
Once you have the credentials for a Service Account and have created a `Disruptive` instance, you can use the API to create new Service Accounts and add them as a members to your projects. See the [`create`](https://vegather.github.io/Disruptive/Disruptive/#disruptive.createserviceaccount(projectid:displayname:basicauthenabled:completion:)) and [`invite`](https://vegather.github.io/Disruptive/Disruptive/#disruptive.invitemember(projectid:roles:email:completion:)) functions for more details.
...
```
[`getAll` documentation](https://vegather.github.io/Disruptive/Disruptive/#disruptive.getdevices(projectid:completion:))
...
func fetchNextPage(pageToken: String?) {
    disruptive.getPage(projectID: "<PROJECT_ID>", pageSize: 25, pageToken: pageToken) { result in
        switch result {
...
```swift
disruptive.get(deviceID: "<DEVICE_ID>") { result in
    ...
}
```
[`get` documentation](https://vegather.github.io/Disruptive/Disruptive/#disruptive.getdevice(projectid:deviceid:completion:))
...
```
[`create` documentation](https://vegather.github.io/Disruptive/Disruptive/#disruptive.createemulateddevice(projectid:devicetype:displayname:labels:completion:))

Here's an example for how to publish a `TemperatureEvent` for an emulated sensor:

```swift
disruptive.publishEvent(
    projectID : "<PROJECT_ID>",
    deviceID  : "<DEVICE_ID>",
...
```
[`publishEvent` documentation](https://vegather.github.io/Disruptive/Disruptive/#disruptive.publishemulatedevent(projectid:deviceid:event:completion:))



##### Fetch Projects & Organizations

Fetching projects lets you optionally filter on both the organization (by identifier) as well as a keyword based query. You can also leave both of those parameters out to fetch all projects available to the authenticated account. The following example will search for projects with a specified organization id (fetched from the `getAll` endpoint for example) that has `Building 1` in its name:

```swift
disruptive.getAll(organizationID: "<ORG_ID>", query: "Building 1") { result in
    ...
}
```
[`getAll` documentation](https://vegather.github.io/Disruptive/Disruptive/#disruptive.getprojects(organizationid:query:completion:))


Here's an example of fetching all the organizations available to the authenticated account:

```swift
disruptive.getAll { result in
    ...
}
```
[`getAll` documentation](https://vegather.github.io/Disruptive/Disruptive/#disruptive.getorganizations(completion:))
